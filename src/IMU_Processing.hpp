...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, int &N)
{
  ...
  if (b_first_frame_) //判断是否为第一帧
  {
    ...
    first_lidar_time = meas.lidar_beg_time;                      //将当前imu帧对应的lidar时间作为初始时间
  }
  ...
  for (const auto &imu : meas.imu) //拿到所有的imu帧
  {
    ...
    // https://blog.csdn.net/weixin_42040262/article/details/127345225 方差迭代计算公式
    ...
    //第二种是
    // cov_acc = cov_acc * (N - 1.0) / N + (cur_acc - mean_acc).cwiseProduct(cur_acc - 上一次的mean_acc) * (N - 1.0) / (N * N);
    // cov_gyr = cov_gyr * (N - 1.0) / N + (cur_gyr - mean_gyr).cwiseProduct(cur_gyr - 上一次的mean_gyr) * (N - 1.0) / (N * N);
...
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI &pcl_out)
{
  ...
  for (auto it_kp = IMUpose.end() - 1; it_kp != IMUpose.begin(); it_kp--)
  {
    ...
    for (; it_pcl->curvature / double(1000) > head->offset_time; it_pcl--) //时间除以1000单位化为s
    {
      ...
      if (it_pcl == pcl_out.points.begin()) //跳出整个循环
        break;
    }
  }
}
